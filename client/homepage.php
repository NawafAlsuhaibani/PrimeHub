...
?>
<?php
$sqlcatid = $conn->query("select cat_id from category order by cat_name asc");
//$sqlcatidarray = $sqlcatid->fetchAll();
//print_r($sqlcatidarray);
while($row = $sqlcatid->fetch()){ // categories id's!
	$id = $row['cat_id'];
	$sqlcatname = $conn->query("select cat_name from category where cat_id = $id");
	while($row1 = $sqlcatname->fetch()){ // category name sql!
		...
		$sqlthr = $conn->query("select thr_id,thr_content,user_id from thread where cat_id = $id");
