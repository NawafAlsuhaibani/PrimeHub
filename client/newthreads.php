...
?>
<?php
$sqlcatid = $conn->query("select cat_id,date_created from category order by date_created DESC");
...
while($row = $sqlcatid->fetch()){ // categories id's!
	...
	while($row1 = $sqlcatname->fetch()){ // category name sql!
		...
		$sqlthr = $conn->query("select thr_id,thr_content,user_id from thread where cat_id = $id ORDER BY date_created DESC");
